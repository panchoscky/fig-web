...
lotes 11-13, banco del Desafío en 187 preguntas).
...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **13 lotes hechos** (`finanzas-01.json` a `finanzas-13.json`, ver detalle en §3 arriba). Cubren: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados`, `macroeconomia` y `fundamentos-corporativos` — 10 temas activos. Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **3 archivos descartados** en total por baja confiabilidad (fórmulas ilegibles / ejercicios sin pauta resuelta) — se prefirió no usarlos antes que inventar. `validar_preguntas.py` → TODO OK, **187 preguntas totales**; balance dificultad 60/88/39 (≈32/47/21). Excels excluidos por instrucción de Francisco. **Aprendizaje del encadenado de lotes:** las fuentes más productivas y seguras son pautas de examen/solemne con comentes V/F ya resueltos (alta densidad conceptual, bajo riesgo de inventar); los archivos de solo enunciados sin solución, o con fórmulas mal extraídas del PDF, es mejor descartarlos que forzar contenido dudoso. **Nuevo aprendizaje (lotes 11-13):** hacer un escaneo rápido de calidad (leer sin comprometerse a extraer) ANTES de construir un lote entero paga bien — de 15 archivos escaneados solo 1 se descartó por completo y 1 quedó parcial, mucho mejor ratio que sin escanear antes. También: el nombre de la carpeta del Drive ("Intro a Finanzas") no siempre coincide con el ramo real del documento — varios archivos escaneados eran en realidad de "Finanzas I" según su propio encabezado; usar el encabezado del documento, no la carpeta, para asignar el ramo. **Quedan pendientes:** el resto de "Intro a Finanzas" y "Finanzas I" (quedan archivos sin escanear en ambas carpetas), luego "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
