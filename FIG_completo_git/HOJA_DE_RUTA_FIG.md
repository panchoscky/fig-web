...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **Lote 1 (2026-07-17):** `finanzas-01.json`, 26 preguntas (`matematica-financiera` + `analisis-financiero`, ambos nuevos), fuentes: *Guía de Materia Concepto de Valor Actual.pdf* + *Ratios Financieros.pdf* + *Ratios.pdf*. **Lote 2 (2026-07-17):** `finanzas-02.json`, 20 preguntas más — 10 amplían `renta-fija` (bonos: tipos, tasa cupón vs mercado, covenants) + 10 crean `riesgo-financiero` (nuevo: riesgo de mercado/crédito/liquidez/contraparte/sistémico), fuentes: 2 resúmenes de cátedra de Iván Abarca. **Lote 3 (2026-07-17):** `finanzas-03.json`, 20 preguntas más desde un único resumen denso (Bonilla) — 10 crean `evaluacion-proyectos` (nuevo: VAN, TIR, TIR modificada, período de recuperación, IR) + 10 amplían `renta-variable` (CAPM, beta, diversificación, covarianza/correlación, Sharpe, eficiencia de mercado). Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. `validar_preguntas.py` → TODO OK, **78 preguntas totales**; balance dificultad 32/32/14 (≈41/41/18, muy cerca del target 40/40/20). Excels excluidos por instrucción de Francisco. **Quedan pendientes:** el resto de "Intro a Finanzas" (67 archivos, ~60 sin procesar tras 3 lotes), luego "Finanzas I" (70), "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
