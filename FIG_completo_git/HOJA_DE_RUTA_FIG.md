...
lotes 14-16, banco del Desafío en 224 preguntas).
...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **16 lotes hechos** (`finanzas-01.json` a `finanzas-16.json`, ver detalle en §3 arriba). Cubren: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados`, `macroeconomia` y `fundamentos-corporativos` — 10 temas activos. Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **4 archivos descartados** en total por baja confiabilidad (fórmulas ilegibles / ejercicios sin pauta resuelta) — se prefirió no usarlos antes que inventar. `validar_preguntas.py` → TODO OK, **224 preguntas totales**; balance dificultad 61/99/64 (≈27/44/29). Excels excluidos por instrucción de Francisco. **Aprendizaje acumulado del encadenado de lotes:** las fuentes más productivas y seguras son pautas de examen/solemne con comentes V/F ya resueltos (alta densidad conceptual, bajo riesgo de inventar); hacer un escaneo rápido de calidad (leer sin comprometerse a extraer) ANTES de construir un lote entero paga muy bien — en los últimos 35 archivos escaneados (15+20) solo 5 resultaron inservibles; el nombre de la carpeta del Drive no siempre coincide con el ramo real del documento, usar el encabezado propio de cada archivo; y cuando varias Ayudantías del mismo profesor repiten el mismo comente o caso (ej. "períodos de gracia", "rancho de sapos"), extraerlo una sola vez y buscar el ángulo distinto en las demás fuentes en vez de duplicar contenido. **Quedan pendientes:** el resto de "Intro a Finanzas" y "Finanzas I" (quedan archivos sin escanear en ambas carpetas), luego "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
