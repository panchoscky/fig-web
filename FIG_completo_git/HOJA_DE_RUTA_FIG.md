...
lotes 18-20, temas nuevos de derivados, banco del Desafío en 266 preguntas).
...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **20 lotes hechos** (`finanzas-01.json` a `finanzas-20.json`, ver detalle en §3 arriba). Cubren 12 temas activos: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados`, `macroeconomia`, `fundamentos-corporativos`, **`derivados-futuros`** y **`derivados-opciones`** (2 temas nuevos, lotes 18-20, confirmados con Francisco). Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **4 archivos descartados** en total por baja confiabilidad. `validar_preguntas.py` → TODO OK, **266 preguntas totales**; balance dificultad 64/115/87 (≈24/43/33). Excels excluidos por instrucción de Francisco. **Aprendizaje acumulado:** las fuentes más productivas son pautas con comentes V/F ya resueltos; escanear antes de extraer paga muy bien; el nombre de la carpeta del Drive no siempre coincide con el ramo real; cuando varias fuentes repiten contenido, extraerlo una sola vez; revisar el backlog de archivos ya escaneados-pero-no-usados antes de escanear más; y cuando el escaneo revela un dominio temático distinto al banco actual (como derivados), pausar y confirmar la taxonomía antes de invertir tiempo construyendo lotes. **Quedan pendientes:** el resto de "Intro a Finanzas" y "Finanzas I" (10 archivos de derivados de la carpeta "Finanzas I" aún sin escanear), luego "Finanzas II" (45, previsiblemente más derivados — opciones avanzadas, Black-Scholes, letras griegas) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
