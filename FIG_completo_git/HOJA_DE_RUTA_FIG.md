...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **Lote 1 hecho (2026-07-17):** `datos/preguntas/finanzas-01.json`, 26 preguntas desde "Intro a Finanzas" (Drive): 14 de `matematica-financiera` (fuente: *Guía de Materia Concepto de Valor Actual.pdf* — VA/VF, anualidades, perpetuidades, tasa efectiva) + 12 de `analisis-financiero` (fuente: *Ratios Financieros.pdf* + *Ratios.pdf* — ROI/ROE, liquidez, endeudamiento, eficiencia). Agregado el ramo `intro-finanzas` y los temas `matematica-financiera`/`analisis-financiero` a `index.json` en el mismo commit. `validar_preguntas.py` → TODO OK, 38 preguntas totales; balance dificultad 17/16/5 (≈45/42/13, cerca del target 40/40/20). Excels excluidos por instrucción de Francisco. **Quedan pendientes:** el resto de "Intro a Finanzas" (67 archivos, ~63 sin procesar), luego "Finanzas I" (70), "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
