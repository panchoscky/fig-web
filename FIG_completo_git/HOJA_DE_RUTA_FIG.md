...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **Lote 1:** `finanzas-01.json`, 26 preguntas (`matematica-financiera` + `analisis-financiero`, ambos nuevos). **Lote 2:** `finanzas-02.json`, 20 preguntas — 10 amplían `renta-fija` + 10 crean `riesgo-financiero`. **Lote 3:** `finanzas-03.json`, 20 preguntas — 10 crean `evaluacion-proyectos` + 10 amplían `renta-variable` (CAPM/portafolios). **Lote 4:** `finanzas-04.json`, 14 preguntas que amplían `analisis-financiero` con interpretación cualitativa de ratios (Contabilidad II). **Lote 5:** `finanzas-05.json`, 10 preguntas que crean `estructura-capital` (Modigliani-Miller, CAPM/WACC, valoración por múltiplos). Un archivo descartado por fórmulas ilegibles en la extracción (regla de no inventar). Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. `validar_preguntas.py` → TODO OK, **102 preguntas totales**; balance dificultad 40/44/18 (≈39/43/18, muy cerca del target 40/40/20). Excels excluidos por instrucción de Francisco. **Quedan pendientes:** el resto de "Intro a Finanzas" (67 archivos, ~55 sin procesar tras 5 lotes), luego "Finanzas I" (70), "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
