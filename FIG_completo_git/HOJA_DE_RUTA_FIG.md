...
|---|---|---|---|
| 1 | **Crear el Apps Script COMPARTIDO del sitio** y pegar su URL en `datos/club.json → config.figEndpoint` | Sonnet | ✅ **RESUELTO (2026-07-18)** — Francisco creó la planilla + el script desde su cuenta y desplegó la Aplicación web (acceso "Cualquier usuario"). URL `/exec` ya pegada en `config.figEndpoint`. **Sin verificar todavía**: este entorno no puede alcanzar `script.google.com` (bloqueado por política de red del sandbox, 403 al CONNECT), así que Francisco debe confirmar manualmente: (1) abrir la URL en el navegador y ver el texto "Endpoint FIG activo", (2) probar el formulario de `postula/index.html` y chequear que aparezca una fila nueva en la pestaña `Postulaciones`, (3) revisar que los nombres de las 3 pestañas de la planilla sean EXACTAMENTE `Postulaciones`, `Ranking`, `Visitas` (si alguna quedó con otro nombre, el script falla silenciosamente al escribir ahí) |
