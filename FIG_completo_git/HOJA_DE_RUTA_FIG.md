...
lote 17, banco del Desafío en 232 preguntas).
...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **17 lotes hechos** (`finanzas-01.json` a `finanzas-17.json`, ver detalle en §3 arriba). Cubren: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados`, `macroeconomia` y `fundamentos-corporativos` — 10 temas activos. Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **4 archivos descartados** en total por baja confiabilidad (fórmulas ilegibles / ejercicios sin pauta resuelta) — se prefirió no usarlos antes que inventar. `validar_preguntas.py` → TODO OK, **232 preguntas totales**; balance dificultad 62/100/70. Excels excluidos por instrucción de Francisco. **Aprendizaje acumulado:** las fuentes más productivas son pautas con comentes V/F ya resueltos; escanear antes de extraer paga muy bien (de los últimos 35+10 archivos escaneados, solo 5 resultaron inservibles); el nombre de la carpeta del Drive no siempre coincide con el ramo real; cuando varias Ayudantías del mismo profesor repiten contenido, extraerlo una sola vez; y **si un archivo se escanea pero no se usa de inmediato, revisar ese backlog antes de escanear más archivos nuevos** (el lote 17 salió íntegro de contenido ya leído en rondas anteriores, cero lecturas nuevas). **Decisión pendiente:** al escanear la carpeta "Finanzas I" aparece contenido de **derivados (forwards/futuros/opciones, Black-Scholes, árboles binomiales) y teoría de portafolios avanzada (CAPM empírico, duración/convexidad)** — 10/10 archivos escaneados excelentes, pero requiere temas nuevos en `index.json` (`derivados-futuros`, `derivados-opciones`) que no se han creado; pendiente de confirmar con Francisco el alcance antes de extraer. **Quedan pendientes:** el resto de "Intro a Finanzas" y "Finanzas I" (quedan archivos sin escanear en ambas, incluida la decisión de derivados), luego "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
