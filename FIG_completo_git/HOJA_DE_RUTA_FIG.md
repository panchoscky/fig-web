...
primer lote de 26 preguntas extraídas, banco del Desafío en 38 preguntas).
...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **10 lotes hechos** (`finanzas-01.json` a `finanzas-10.json`, ver detalle en §3 arriba). Cubren: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados` y `macroeconomia` — 9 temas activos. Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **2 archivos descartados** por baja confiabilidad (fórmulas ilegibles / ejercicios sin pauta resuelta) — se prefirió no usarlos antes que inventar. `validar_preguntas.py` → TODO OK, **142 preguntas totales**; balance dificultad 50/66/26 (≈35/46/18). Excels excluidos por instrucción de Francisco. **Aprendizaje del encadenado de lotes:** las fuentes más productivas y seguras son pautas de examen/solemne con comentes V/F ya resueltos (alta densidad conceptual, bajo riesgo de inventar); los archivos de solo enunciados sin solución, o con fórmulas mal extraídas del PDF, es mejor descartarlos que forzar contenido dudoso. **Quedan pendientes:** el resto de "Intro a Finanzas" (67 archivos, ~44 sin procesar tras 10 lotes), luego "Finanzas I" (70), "Finanzas II" (45) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
