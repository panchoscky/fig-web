...
lotes 21-22, banco del Desafío en 287 preguntas).
...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **22 lotes hechos** (`finanzas-01.json` a `finanzas-22.json`, ver detalle en §3 arriba). Cubren 12 temas activos: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados`, `macroeconomia`, `fundamentos-corporativos`, `derivados-futuros` y `derivados-opciones`. Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **4 archivos descartados** en total por baja confiabilidad. `validar_preguntas.py` → TODO OK, **287 preguntas totales**; balance dificultad 64/121/102 (≈22/42/36). Excels excluidos por instrucción de Francisco. **Aprendizaje acumulado:** las fuentes más productivas son pautas con comentes V/F ya resueltos; escanear antes de extraer paga muy bien (los últimos 40 archivos de "Finanzas I" escaneados en 4 rondas de 10 salieron prácticamente todos aprovechables); el nombre de la carpeta del Drive no siempre coincide con el ramo real; cuando varias fuentes repiten contenido, extraerlo una sola vez; revisar el backlog de archivos ya escaneados-pero-no-usados antes de escanear más; y cuando el escaneo revela un dominio temático distinto al banco actual (como derivados), pausar y confirmar la taxonomía antes de invertir tiempo construyendo lotes. **Quedan pendientes:** el resto de "Intro a Finanzas" y "Finanzas I" (carpeta con 70 archivos totales, quedan varios sin escanear — mucho contenido de opciones avanzadas y Black-Scholes con letras griegas todavía no tocado), luego "Finanzas II" (45, previsiblemente más derivados) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
