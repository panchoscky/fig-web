...
| `juego/index.html` | ✅ Funcional | "El Rally del Toro": runner canvas con un **toro dorado dibujado a mano** (silueta embistiendo inspirada en el logo, galope de 4 patas, cola y cuernos animados — ya no se usa la imagen del logo en el canvas); velas rojas y burbujas como obstáculos; el portafolio crece con multiplicador creciente; VENDER asegura el puntaje — filosofía "saber cuándo salir". Ranking: lee `config.juegoEndpoint` de `datos/club.json` (Apps Script doPost/doGet, código listo en `HOJA_DE_RUTA_FIG.md` P1-10b) y muestra "Ranking global"; si el endpoint está vacío o el fetch falla, cae automático a localStorage por navegador ("Ranking de este navegador"). Falta que Francisco despliegue el Apps Script y pegue la URL. Enlazarlo en cualquier página en mantención/espera |
