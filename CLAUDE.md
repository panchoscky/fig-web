...
| `fiw/index.html` | ⚠️ Placeholder + **enlaces ocultos temporalmente** | Estructura y datos completos (ahora con 3 co-fundadoras: Delia Avilán, Gabriela Domínguez y Victoria Espinoza), pero **colores de marca aún no confirmados** por el equipo FIW — usa un oro rosa provisional. Sin fotos en `fotos/fiw/` todavía. **2026-07-24, pedido de Francisco**: se ocultaron (comentario `<!-- FIW_TEMP_OCULTO -->`, NO se borró nada) todos los botones/enlaces que llevan a esta página — nav desktop y móvil de `index.html`, la tarjeta del desk 04 en §Áreas, el footer de `index.html`, y los nav de `404.html`, `postula/index.html` y `eventos/index.html` — porque el área se publica recién en una etapa posterior al lanzamiento de mañana. La página en sí sigue intacta y accesible por URL directa; para reactivarla, buscar `FIW_TEMP_OCULTO` en esos 5 archivos y descomentar |
...
| `torneo/index.html` | ✅ Datos reales cargados | `datos/torneo.json` ya existe (60 equipos reales del Excel Oficial FIG + LinkedIn de Copia de Inscripciones — ver nota de continuidad abajo). La página salió del modo DEMO sola. Overlay con gráfico de 3 líneas (retorno equipo/promedio/ACWI — el ACWI queda vacío por ahora, no hay benchmark en el Excel usado). Tarjetas: Feed PNG, **Story PNG**, LinkedIn PNG, HTML y **videos animados** Feed/Story con intro (logo→nombre→colaboradores→ficha). En celular, los botones de tarjetas **comparten directo con el panel nativo del sistema** (`navigator.share`, Instagram queda a un toque) en vez de forzar una descarga; en iPhone la tarjeta Story además intenta abrir Instagram directo en el compositor de Historias (truco de portapapeles + `instagram-stories://`, con fallback automático). Grabación de video a 24fps con progreso visible en el botón. Logos de colaboradores en hero y tarjetas. La línea temporal de §Metodología integra las actividades del club desde `datos/eventos.json` (tags por tipo + descripción al hover, tarea #16 ✅). **Sin trayectoria en semana 1 (2026-07-24, pedido de Francisco)**: con solo 1 semana de `historial` en `torneo.json`, `seriesRet()`/`cardSeries()` no tienen datos para trazar el gráfico de 3 líneas — el overlay ya mostraba el aviso honesto "TRAYECTORIA DISPONIBLE DESDE LA SEGUNDA SEMANA PUBLICADA", pero las tarjetas Feed/Story/LinkedIn/HTML y los videos dejaban un hueco vacío en su lugar (se veía como un gráfico roto). Se agregó `chartPlaceholder()` (canvas) y su equivalente en la tarjeta HTML con el mismo aviso, para que las descargas también expliquen por qué falta el gráfico en vez de mostrar un espacio en blanco — se resuelve solo cuando exista una segunda semana real de datos, no requiere tocar código de nuevo. También se quitó el guión "—" que aparecía junto a la posición de cada equipo en la tabla (`renderLb`, indicador de cambio semanal): con una sola semana el delta siempre es 0/indefinido, así que ese guión salía en las 59 filas sin aportar información; ahora esa marca solo se muestra cuando de verdad hay un cambio de posición semana a semana |
