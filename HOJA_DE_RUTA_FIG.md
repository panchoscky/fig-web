...
| Q2 | **Extracción de preguntas por lotes** | Sonnet | 🔄 En curso — **23 lotes hechos** (`finanzas-01.json` a `finanzas-23.json`, ver detalle en §3 arriba). Cubren 12 temas activos: `matematica-financiera`, `analisis-financiero`, `renta-fija`, `riesgo-financiero`, `evaluacion-proyectos`, `renta-variable`, `estructura-capital`, `historia-mercados`, `macroeconomia`, `fundamentos-corporativos`, `derivados-futuros` y `derivados-opciones`. Ramos/temas nuevos agregados a `index.json` en el mismo commit que se usan, como indica LEEME.md. **4 archivos descartados** en total por baja confiabilidad. `validar_preguntas.py` → TODO OK, **300 preguntas totales** (hito redondo); balance dificultad 64/125/111 (≈21/42/37). Excels excluidos por instrucción de Francisco. **Aprendizaje acumulado:** las fuentes más productivas son pautas con comentes V/F ya resueltos; escanear antes de extraer paga muy bien (los últimos 50 archivos de "Finanzas I"/"Finanzas II" escaneados en 5 rondas de 10 salieron prácticamente todos aprovechables, aunque las últimas rondas empiezan a mostrar más contenido duplicado entre sí — señal de que esa carpeta se está agotando); el nombre de la carpeta del Drive no siempre coincide con el ramo real; cuando varias fuentes repiten contenido, extraerlo una sola vez; revisar el backlog de archivos ya escaneados-pero-no-usados antes de escanear más; y cuando el escaneo revela un dominio temático distinto al banco actual (como derivados), pausar y confirmar la taxonomía antes de invertir tiempo construyendo lotes. **Quedan pendientes:** el resto de "Intro a Finanzas" y "Finanzas I" (carpeta con 70 archivos totales, quedan pocos sin escanear y con creciente solapamiento), luego "Finanzas II" (45, ya se detectó que también vive parcialmente dentro de la carpeta "Finanzas I" del Drive — revisar bien los encabezados) y "APF" (7) — ver orden recomendado en `Q1_INVENTARIO_FINANZAS.md` |
